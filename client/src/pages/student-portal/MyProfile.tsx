...
import { User, Phone, Mail, MapPin, Calendar, BookOpen, Layers, Shield } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Registered student identity, guardian contact info, and academic registration.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 pb-6 border-b border-slate-100 dark:border-slate-800 text-center sm:text-left">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3">
              ...
              <span className="font-mono text-xs font-bold px-3 py-1 bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-300 rounded-full border border-blue-200 dark:border-blue-800">
                {student.studentId}
              </span>
              <Badge variant="success">Active Learner</Badge>
            </div>
            <p className="text-sm font-semibold text-blue-600 dark:text-blue-400 mt-1">
              Course: {student.course?.name} &bull; Batch: {student.batch?.name || 'Cohort'}
            </p>
            <p className="text-xs text-slate-400 mt-2">
              Admitted: {student.admissionDate} &bull; Gender: {student.gender}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-2.5 text-xs">
